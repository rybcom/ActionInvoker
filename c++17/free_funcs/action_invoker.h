...
namespace action_invoker
{
	...
	void process_deferred_actions(double currentTime)
	{
		...
		ranges::for_each_where(deferred_actions, action_expired, execute_action);
		ranges::erase_if(deferred_actions, action_expired);

		ranges::for_each(deferred_continous_actions, execute_action_if_is_in_progress);
		ranges::erase_if(deferred_continous_actions, continous_action_expired);
	}
